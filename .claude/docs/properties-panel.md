...
**PDF export (`Export/utils/pdf/`, hybrid layers).** `createExportFile` routes `ExportFormat.pdf` to `createPdfBlob(nodeId, rasterScale, ignoreOverlappingLayers, imageResampling)` (raster scale = `max(rowScale, PDF_MIN_RASTER_SCALE = 2)`; the page itself is always 1px = 1pt at the node's rotated bounds, so the row's Scale doesn't resize the page). `isRasterExportFormat` became `isSupportedExportFormat` (png/jpeg/pdf; svg still skipped). Pipeline: `getExportRenderNodes` gives the paint-ordered node list; `getPdfLayers` walks it and classifies each node into one of three layer kinds, preserving paint order: a `text` layer (eligible straight text), a `vector` layer (eligible Rectangle/Frame) or a `raster` layer (every other node, consecutive ones merged into one layer). A raster layer is rendered through the existing export renderer with the optional `includeNodeIds` filter (`renderNodeForExport(..., includeNodeIds)` → request field → `resolveExportRenderRequest` filters `nodesToDraw`), PNG-encoded and embedded full-page with `pdf-lib`.
...
- **Vector layers** (`drawPdfShape`): reuses the renderer's own geometry — `getBoxFillPolygon` for the fill and `getBoxStrokeRingPolygons` for the stroke (every stroke mode/dash/per-side width already comes out as ring polygons) — so shapes match the canvas exactly. Each solid paint (in `getFillsInPaintOrder`, opacity scaled by `getEffectiveOpacity` × paint opacity/100) becomes one even-odd filled path (`drawPdfPolygons`, raw `pdf-lib` operators; opacity via a cached `ExtGState`, `getPdfGraphicsState`). `canExportShapeAsVector` keeps it to: only visible solid paints with normal blend, no visible effects, no legacy `strokeColor`, stroke mode basic (brush/dynamic stay raster), safe ancestors. Curves are the renderer's polylines (`ROUNDED_RECT_CORNER_SEGMENTS = 8` per corner), so big radii look faceted when zoomed far in.
...
Only Inter exists today; when the backend serves other fonts, `loadPdfFontBytes` is the single place to turn into a per-family/weight lookup (text with an unavailable font should then fall back to raster/curves, not be substituted). Not done yet: vector ellipse/polygon/star/line/path nodes, gradients as PDF shadings, text-on-path as curves; shadow-under-text (Figma draws it as an image below selectable text) is naturally what the layer split already yields once text nodes carry effects.
