...
- **Vector layers** (`drawPdfShape`, `switch (node.type)` dispatching to `drawPdfBoxShape`/`drawPdfLineShape`/`drawPdfSimpleShape`): eligibility (`canExportShapeAsVector`) dispatches the same way, to `canExportBoxShapeAsVector`/`canExportLineAsVector`/`canExportSimpleShapeAsVector`.
  - **Frame/Rectangle** (`drawPdfBoxShape`/`canExportBoxShapeAsVector`): reuses the renderer's own geometry — `getBoxFillPolygon` for the fill and `getBoxStrokeRingPolygons` for the stroke (every stroke mode/dash/per-side width already comes out as ring polygons). Each solid paint (in `getFillsInPaintOrder`, opacity scaled by `getEffectiveOpacity` × paint opacity/100) becomes one even-odd filled path. Eligibility: only visible solid paints with normal blend, no visible effects, no legacy `strokeColor`, stroke mode basic (brush/dynamic stay raster), safe ancestors. Curves are the renderer's polylines (`ROUNDED_RECT_CORNER_SEGMENTS = 8` per corner).
...
Only Inter exists today; when the backend serves other fonts, `loadPdfFontBytes` is the single place to turn into a per-family/weight lookup (text with an unavailable font should then fall back to raster/curves, not be substituted). Not done yet: the general bezier pen-tool shape (`NodeType.vector` — multi-face fills/holes, variable-width strokes) as vector paths, gradients as PDF shadings, text-on-path as curves; shadow-under-text (Figma draws it as an image below selectable text) is naturally what the layer split already yields once text nodes carry effects.
