...
A Noise effect stores `noiseType` (Mono / Duo / Multi, default Mono), `noiseSize` (blob size in node units, default 0.5; the Y field is read-only and shows the same value), `density` (0-100, default 100, shown as `100%`), color, opacity and blend mode; `getEffectNoise` and `getEffectFieldValue` resolve the defaults. `getEffectPanelLayout` (a `switch` on the type) gives it the blend mode droplet, the `EffectNoiseTypeToggle` (Mono / Duo / Multi; only Mono is implemented, the toggle is fixed on Mono), the fields from `EFFECT_NOISE_FIELDS` (`isReadOnly`, `unit: '%'`, `ariaKey` for the Y field) and the color row. `EFFECT_FIELD_MAX` clamps the density on blur, scrub and arrow keys, and `getEffectNumberFromInput` accepts a trailing `%`.
