...
Sits after Layout guide on `Frame.tsx` and after Effects on `Rectangle.tsx`; the same component is also rendered (unchanged) from `NoSelection.tsx`. **UI-only so far** — clicking the plus just pushes a default row into local component state (`useExportSection`, plain `useState<TExportSetting[]>`, reset via a `useEffect` keyed on `node?.id`); there is no `exportSettings` node field yet and no actual PNG/SVG/PDF generation. `useExportSection` derives `node` from `selectSelectedNodes` (only when exactly one node is selected) purely to decide whether to show the "Export {name}" button/preview below the row list — it doesn't gate the row list itself, which is why the same component also works from `NoSelection` (no node, rows still addable, just no button/preview).
