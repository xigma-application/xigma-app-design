...
- `continueDrag/getMatchedPairDragGuides.ts` — the drag-time gate: `getEligibleDraggedEntries` for
  ...
  either on top just doubles the strokes and the gap numbers. The chain-gap **delta** is still
  applied; only its guides are suppressed.
...
[[constraints]] — the dashed blue constraint guide lines + centre `×` marker drawn for a
sole-selected constraint-eligible frame child, right after `drawSelectionOutline` in `drawScene`.
