...
`drawBoxLeafNodeFill` calls `drawBoxEffects` right after the box paints (Frame / Rectangle only, `node.effects`), so the order is fill, effects, children, stroke. Only visible `EffectType.innerShadow` entries are drawn (`drawBoxInnerShadow`, one per effect, in array order); Drop shadow is still a no-op.
