...
- `drawScene.ts` — first derives its own locals straight off `refs` (`draftShape`, `marqueeRect`,
  ...
  **background → committed nodes → hover outline → selection outline → selection size label →
  ...
  evicted first), keyed by `JSON.stringify([node, zoom])` (`getFrameNameLabelCacheKey.ts`, the same
  "stringify the whole node" approach as the layer-blur cache's `getBlurCacheKey`) and only rebuilds
  when that key changes — a pure pan, or any redraw triggered by something unrelated elsewhere in
