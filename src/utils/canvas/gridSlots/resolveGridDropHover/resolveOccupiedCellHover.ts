...
export const resolveOccupiedCellHover = (
  ...
  if (!owner || owner.columnSpan * owner.rowSpan <= 1) {
    const cellLeft = layout.padding.paddingLeft + getGridTrackOffset(layout.columnSizes, layout.columnGap, column);
    const isLeftHalf = framePoint.x - cellLeft < layout.columnSizes[column] / 2;
