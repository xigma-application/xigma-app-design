...
// utils
import { drawStarCornerRadiusHandle } from '../drawStarCornerRadiusHandle';
...
describe('drawStarCornerRadiusHandle', () => {
  it('should draw a fill and a stroke pass for the single handle', () => {
    ...
    // before
    drawStarCornerRadiusHandle(gl, program, buffer, STAR_BOUNDS, 5, 0.5, 10, '#0d99ff', 100, 100, IDENTITY_VIEWPORT, 0);
  ...
  it('should position the handle moved toward the center by the radius scaled by the setback multiplier', () => {
    // mock
    const gl = createGlMock();
    const program = {} as WebGLProgram;
    const buffer = {} as WebGLBuffer;

    // before — top vertex of a 100x100 5-point star sits at (50, 0); radius 10 moves it by ~22.6
    drawStarCornerRadiusHandle(gl, program, buffer, STAR_BOUNDS, 5, 0.5, 10, '#0d99ff', 100, 100, IDENTITY_VIEWPORT, 0);
    ...
    expect(vertices[1]).toBeCloseTo(22.595515, 5);
  });
  ...
  it('should rotate the handle position around the bounds center when rotation is given', () => {
    ...
    // before
    drawStarCornerRadiusHandle(gl, program, buffer, STAR_BOUNDS, 5, 0.5, 10, '#0d99ff', 100, 100, IDENTITY_VIEWPORT, 90);

    // result — the handle (50, ~22.6) rotated 90deg around the center (50, 50) swings to (~77.4, 50)
    const [firstFillCall] = (gl.bufferData as ReturnType<typeof vi.fn>).mock.calls;
    const vertices: Float32Array = firstFillCall[1];

    expect(vertices[0]).toBeCloseTo(77.404485, 5);
