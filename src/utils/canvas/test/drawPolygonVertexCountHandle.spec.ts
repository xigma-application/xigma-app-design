...
// utils
import { drawPolygonVertexCountHandle } from '../drawPolygonVertexCountHandle';
...
describe('drawPolygonVertexCountHandle', () => {
  ...
  it('should position the handle on the polygon vertex index 1', () => {
    // mock
    const gl = createGlMock();
    const program = {} as WebGLProgram;
    const buffer = {} as WebGLBuffer;
    ...
    expect(vertices[0]).toBeCloseTo(93.30127, 4);
    expect(vertices[1]).toBeCloseTo(75, 4);
  });
  ...
  it('should rotate the handle position around the bounds center when rotation is given', () => {
    ...
    expect(vertices[0]).toBeCloseTo(25, 4);
    expect(vertices[1]).toBeCloseTo(93.30127, 4);
  });
  ...
  it('should flip the handle position when flipX/flipY are set', () => {
    ...
    expect(vertices[0]).toBeCloseTo(6.69873, 4);
    expect(vertices[1]).toBeCloseTo(25, 4);
  });
});
