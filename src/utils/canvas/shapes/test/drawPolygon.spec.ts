...
describe('drawPolygon', () => {
  it('should draw a triangle fan when fill is given', () => {
    ...
    // before
    drawPolygon(gl, program, buffer, { fill: '#ffffff', height: 10, sides: 5, width: 10, x: 0, y: 0 }, 100, 100, IDENTITY_VIEWPORT);
  ...
  it('should draw an outline when stroke is given', () => {
    ...
    // before
    drawPolygon(gl, program, buffer, { height: 10, sides: 5, stroke: '#ffffff', width: 10, x: 0, y: 0 }, 100, 100, IDENTITY_VIEWPORT);
  ...
  it('should draw nothing when neither fill nor stroke is given', () => {
    ...
    // before
    drawPolygon(gl, program, buffer, { height: 10, sides: 5, width: 10, x: 0, y: 0 }, 100, 100, IDENTITY_VIEWPORT);
  ...
  it('should upload the viewport transform and resolution as uniforms', () => {
    ...
    // before
    drawPolygon(gl, program, buffer, { fill: '#ffffff', height: 10, sides: 5, width: 10, x: 0, y: 0 }, 100, 200, {
      x: 5,
      y: 15,
      zoom: 2,
    });
  ...
  it('should center the fan on the bounding rect', () => {
    ...
    // before
    drawPolygon(gl, program, buffer, { fill: '#ffffff', height: 20, sides: 4, width: 10, x: 0, y: 0 }, 100, 100, IDENTITY_VIEWPORT);
