...
import { collapseCuspEdges } from './collapseCuspEdges/collapseCuspEdges';
...
export const getGlyphEdgeLoops = (path: Path): TLoopEdge[][] => {
  ...
  const loops = final.edges.length > 0 ? [...final.loops, final.edges] : final.loops;

  return loops.map(collapseCuspEdges);
};
