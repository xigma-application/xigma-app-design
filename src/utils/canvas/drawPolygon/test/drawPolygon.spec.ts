...
describe('drawPolygon', () => {
  it('should delegate to the rounded triangle-fan rendering when cornerRadius is set', () => {
    ...
    drawPolygon(
      ...
      program,
      buffer,
      { cornerRadius: 10, fill: '#ffffff', height: 100, sides: 3, width: 100, x: 0, y: 0 },
      100,
      100,
      IDENTITY_VIEWPORT,
      false,
  ...
  it('should delegate to the plain fan rendering when cornerRadius is 0', () => {
    ...
    drawPolygon(
      ...
      program,
      buffer,
      { cornerRadius: 0, fill: '#ffffff', height: 10, sides: 5, width: 10, x: 0, y: 0 },
      100,
      100,
      IDENTITY_VIEWPORT,
      false,
    ...
    // result — the raw pentagon fan has 5 vertices + center + closing point, not a rounded arc-fan
    expect(gl.drawArrays).toHaveBeenCalledWith(gl.TRIANGLE_FAN, 0, 7);
  });
