...
describe('getPolygonCornerRadiusHandleAtPoint', () => {
  ...
  it('should detect the handle on an unrotated triangle', () => {
    // mock — top vertex of a 100x100 triangle sits at (50, 0); radius 15 moves the handle to (50, 15)
    const node = polygon('a', 0, 0, 100, 100, 3, 15);

    // result
    expect(getPolygonCornerRadiusHandleAtPoint({ x: 50, y: 15 }, [node], IDENTITY_VIEWPORT)).toEqual({
  ...
  it('should be grabbable even while the corner radius is still 0, at the zero-state offset position', () => {
    // mock — max radius (25) clamps the zero-state offset for a 100x100 triangle
    const node = polygon('a', 0, 0, 100, 100, 3, 0);

    // result
    expect(getPolygonCornerRadiusHandleAtPoint({ x: 50, y: 25 }, [node], IDENTITY_VIEWPORT)).toMatchObject({ nodeId: 'a' });
  });
  ...
  it('should identify the handle by its physically rotated position', () => {
    // mock — the handle of an unrotated 100x100/radius-15 triangle sits at local (50, 15); rotating
    // the node 90deg around its center (50, 50) swings that same physical point to world (85, 50)
    const node = polygon('a', 0, 0, 100, 100, 3, 15, 90);
    const center = { x: 50, y: 50 };

    // result
    expect(getPolygonCornerRadiusHandleAtPoint(rotatePoint({ x: 50, y: 15 }, center, 90), [node], IDENTITY_VIEWPORT)).toMatchObject({
      nodeId: 'a',
    });
  });
  ...
  it('should widen the hit tolerance in world units as the viewport zooms out', () => {
    ...
    expect(getPolygonCornerRadiusHandleAtPoint({ x: 200, y: 68 }, [node], IDENTITY_VIEWPORT)).toBeNull();
    expect(getPolygonCornerRadiusHandleAtPoint({ x: 200, y: 68 }, [node], { x: 0, y: 0, zoom: 0.5 })).toMatchObject({ nodeId: 'a' });
  });
