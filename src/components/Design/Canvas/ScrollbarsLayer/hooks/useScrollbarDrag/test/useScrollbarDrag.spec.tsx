...
const captureDragStart = (
  ...
  const geo = getScrollGeometry(CANVAS_RECT, createLayout(leftPanelWidth, rightPanelWidth), selectOrderedNodes(state), viewport);
...
describe('useScrollbarDrag', () => {
  ...
  it('should render identically right after release as on the last frame of the drag, at the same viewport', () => {
    ...
    const duringGeo = getScrollGeometry(CANVAS_RECT, createLayout(), selectOrderedNodes(store.getState()), duringViewport);
    ...
    const afterGeo = getScrollGeometry(CANVAS_RECT, createLayout(), selectOrderedNodes(store.getState()), selectViewport(store.getState()));
