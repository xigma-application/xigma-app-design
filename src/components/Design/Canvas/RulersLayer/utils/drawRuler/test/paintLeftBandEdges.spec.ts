...
describe('paintLeftBandEdges', () => {
  it('should mark and label both frame edges, rotated, nudged toward the band interior', () => {
    // mock
    const ctx = createFakeContext();

    // action
    paintLeftBandEdges(ctx as unknown as CanvasRenderingContext2D, band(), 300, 600);
  ...
  it('should draw nothing when the band carries no edge labels', () => {
    ...
    // action
    paintLeftBandEdges(ctx as unknown as CanvasRenderingContext2D, band({ edges: null }), 0, 600);
  ...
  it('should skip an edge that sits outside the drawable strip', () => {
    ...
    // action
    paintLeftBandEdges(ctx as unknown as CanvasRenderingContext2D, band({ fromPx: 5, toPx: 900 }), 0, 600);
