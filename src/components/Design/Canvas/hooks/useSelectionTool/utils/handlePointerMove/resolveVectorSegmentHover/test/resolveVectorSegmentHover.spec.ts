...
describe('resolveVectorSegmentHover', () => {
  ...
  it('should clear the hover state and reset the cursor, never highlighting a segment, while Erase is active', () => {
    // mock — Erase owns its own brush preview (resolveVectorEraseHover.ts), so the generic
    // blue highlight/white insert-point dot must stay fully inert even over a segment
    const nodeId = addVectorNode();
    store.dispatch(setVectorEditingNodeIds([nodeId]));
