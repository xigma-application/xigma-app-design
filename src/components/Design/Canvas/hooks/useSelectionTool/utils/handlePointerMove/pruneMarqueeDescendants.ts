// store
import { getIsDescendantOfMovedIdSet } from 'store/design/utils/handleMoveNodes/getIsDescendantOfMovedIdSet';
import { isClickThroughFrame } from 'store/design/utils/nodeHierarchy/isClickThroughFrame';

// types
import { TSceneNode } from 'types/design/types';

export const pruneMarqueeDescendants = (collided: TSceneNode[], nodesById: Record<string, TSceneNode>): TSceneNode[] => {
  const collidedFrameIds = new Set(collided.filter((node) => isClickThroughFrame(node, nodesById)).map((node) => node.id));
  return collided.filter((node) => !getIsDescendantOfMovedIdSet(node.parentId, collidedFrameIds, nodesById));
};
