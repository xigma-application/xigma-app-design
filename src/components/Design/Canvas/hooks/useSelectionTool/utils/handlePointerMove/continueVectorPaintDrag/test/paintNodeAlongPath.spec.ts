...
describe('paintNodeAlongPath behaviors', () => {
  it('should paint every still-untouched face the path crosses and return every crossed face key', () => {
    ...
    // before
    const faceKeys = paintNodeAlongPath(dispatch, node, [{ x: 70, y: 30 }], solid('#00ff00'), false, touchedLoopKeys);
  ...
  it('should skip a face already touched earlier in the same stroke', () => {
    ...
    // before
    paintNodeAlongPath(dispatch, node, [{ x: 70, y: 30 }], solid('#00ff00'), false, touchedLoopKeys);
    dispatch.mockClear();
    paintNodeAlongPath(dispatch, node, [{ x: 75, y: 25 }], solid('#00ff00'), false, touchedLoopKeys);
  ...
  it('should remove an already-filled face instead of adding it when remove mode is on', () => {
    ...
    // before
    paintNodeAlongPath(dispatch, node, [{ x: 70, y: 30 }], solid('#00ff00'), true, touchedLoopKeys);
  ...
  it('should bake a virtual crossing the newly-touched face depends on into real, persisted segments/vertices', () => {
    ...
    // before
    const faceKeys = paintNodeAlongPath(dispatch, node, [{ x: 50, y: 25 }], solid('#00ff00'), false, touchedLoopKeys);
