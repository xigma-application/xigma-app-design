...
describe('getDraggedBlockPreviewMeta', () => {
  it('keys the member slots by node id, names the grabbed member, and carries the clamp box + contiguous flag', () => {
    // action
    const meta = getDraggedBlockPreviewMeta(['b', 'c'], 'b', SLOTS, CLAMP_BOX, false);
  ...
  it('names the second member when it is the one grabbed', () => {
    // action
    const meta = getDraggedBlockPreviewMeta(['b', 'c'], 'c', SLOTS, CLAMP_BOX, false);
  ...
  it('marks the block contiguous when told to', () => {
    // action
    const meta = getDraggedBlockPreviewMeta(['b', 'c'], 'c', SLOTS, CLAMP_BOX, true);
  ...
  it('falls back to the first member when the grabbed id is null or not in the block', () => {
    // action
    expect(getDraggedBlockPreviewMeta(['b', 'c'], null, SLOTS, CLAMP_BOX, false).draggedGrabbedId).toBe('b');
    expect(getDraggedBlockPreviewMeta(['b', 'c'], 'zzz', SLOTS, CLAMP_BOX, false).draggedGrabbedId).toBe('b');
  });
});
