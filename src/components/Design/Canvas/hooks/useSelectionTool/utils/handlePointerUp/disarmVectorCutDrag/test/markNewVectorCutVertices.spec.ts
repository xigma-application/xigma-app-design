...
describe('markNewVectorCutVertices', () => {
  ...
  it('should mark every vertex id present after the cut but not before, for a node that stayed open', () => {
    ...
    // before
    markNewVectorCutVertices(canvasRefs, beforeNodes, [nodeId]);
  ...
  it('should mark nothing for a node id that did not exist before the cut (a brand-new sibling from a full divide)', () => {
    // mock — beforeNodes has nothing under this id at all
    const nodeId = addVectorNode();
    const emptyBeforeNodes: Record<string, TSceneNode> = {};
    const canvasRefs = createCanvasRefs();

    // before
    markNewVectorCutVertices(canvasRefs, emptyBeforeNodes, [nodeId]);

    // result
    expect(canvasRefs.newVectorCutVertexIdsRef.current).toEqual(new Set());
  });

  it('should mark nothing for a node id that no longer exists after the cut', () => {
    // mock
    const nodeId = addVectorNode();
    const beforeNodes = store.getState().design.nodes;
    const canvasRefs = createCanvasRefs();
    ...
    markNewVectorCutVertices(canvasRefs, beforeNodes, ['stale-id']);
  ...
  it('should mark vertices across several open nodes at once', () => {
    ...
    // before
    markNewVectorCutVertices(canvasRefs, beforeNodes, [firstNodeId, secondNodeId]);
