...
describe('armSmartSelectionGapOnPointerDown', () => {
  ...
  it('should look up the current nodes-by-id map and forward it to the hit test', () => {
    ...
    // before
    armSmartSelectionGapOnPointerDown({ canvas, canvasRefs, event, point, smartSelectionNodes, viewport } as never);

    // result
    expect(getSmartSelectionGapHandleAtPointMock).toHaveBeenCalledWith(point, smartSelectionNodes, viewport, selectNodes(store.getState()));
  });
});
