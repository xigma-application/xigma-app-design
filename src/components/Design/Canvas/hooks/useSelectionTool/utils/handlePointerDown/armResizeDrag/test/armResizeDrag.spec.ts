...
describe('armResizeDrag', () => {
  ...
  it('should resize a rotated group as one rigid box — a single origin for the group node', () => {
    // mock
    const canvas = createCanvas();
    const resizeDragRef = createResizeDragRef();
    ...
    // result
    expect(Object.keys(resizeDragRef.current?.nodeOrigins ?? {})).toEqual([groupId]);
    expect(resizeDragRef.current?.nodeOrigins[groupId]).toMatchObject({ rotation: 25 });
  });

  it('should expand a selected group into per-child origins so resizing scales every child', () => {
    // mock
    const canvas = createCanvas();
    const resizeDragRef = createResizeDragRef();
    ...
    const childIds = rootOrder.slice(-2);
    ...
    // result
    expect(Object.keys(resizeDragRef.current?.nodeOrigins ?? {})).toEqual(childIds);
    expect(resizeDragRef.current?.nodeOrigins[groupId]).toBeUndefined();
  });
});
