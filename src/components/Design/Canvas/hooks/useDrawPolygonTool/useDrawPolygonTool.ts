...
export const useDrawPolygonTool = (refs: TCanvasRefs, { fill, name, sides, tool }: TPolygonToolConfig): void => {
  ...
  const onPointerUp = (canvas: HTMLCanvasElement, event: PointerEvent): void =>
    handlePointerUp(canvas, event, dispatch, appStore, refs, viewport, startRef, candidateShapesRef, fill, name, sides);
  ...
  }, [activeTool, appStore, canvasRef, dispatch, fill, name, refs, sides, tool, viewport]);
};
