...
describe('handlePointerMove', () => {
  it('should do nothing when the drag has not started', () => {
    ...
    // before
    handlePointerMove(createCanvas(), pointerEvent(50, 50), refs, IDENTITY_VIEWPORT, { current: null }, { current: [] }, '#ff0000', 3);

    // result
    expect(refs.draftRef.current).toBeNull();
  });

  it('should update the draft polygon and alignment guide as the pointer moves', () => {
    // mock
    const refs = createCanvasRefs();
