...
describe('drawVectorTangentHandles', () => {
  ...
  it('should draw the tangentStart end of a segment when a real tangentStart is set', () => {
    ...
    // before
    drawVectorTangentHandles({} as WebGL2RenderingContext, {} as WebGLProgram, {} as WebGLBuffer, node, null, 200, 150, IDENTITY_VIEWPORT);
    ...
    expect(drawTangentHandleMock).toHaveBeenCalledWith({}, {}, {}, node.vertices.v1, { x: 5, y: 0 }, 5, false, 200, 150, IDENTITY_VIEWPORT);
  });
  ...
  it('should draw a default preview handle for the tangentStart end when only tangentEnd is set, without mutating the segment', () => {
    ...
    expect(drawTangentHandleMock).toHaveBeenCalledWith({}, {}, {}, node.vertices.v1, { x: 1, y: 0 }, 5, false, 200, 150, IDENTITY_VIEWPORT);
    expect(drawTangentHandleMock).toHaveBeenCalledWith({}, {}, {}, node.vertices.v2, { x: 8, y: 0 }, 5, false, 200, 150, IDENTITY_VIEWPORT);
  ...
  it('should mark only the hovered end as hovered when it matches the segment id and end', () => {
    ...
    // result
    expect(drawTangentHandleMock).toHaveBeenCalledWith({}, {}, {}, node.vertices.v1, { x: 5, y: 0 }, 5, true, 200, 150, IDENTITY_VIEWPORT);
    expect(drawTangentHandleMock).toHaveBeenCalledWith({}, {}, {}, node.vertices.v2, { x: 5, y: 0 }, 5, false, 200, 150, IDENTITY_VIEWPORT);
  });
