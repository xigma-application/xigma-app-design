...
export const blurBoxEffectTexture = (
  ...
  drawEffectBlurPass(gl, imageContext.blurProgram, imageContext.blurBuffer, source.texture, [1, 0], radius, source.width, source.height);

  gl.bindFramebuffer(gl.FRAMEBUFFER, source.framebuffer);
  gl.viewport(0, 0, source.width, source.height);
  gl.clearColor(0, 0, 0, 0);
  gl.clear(gl.COLOR_BUFFER_BIT);
  drawEffectBlurPass(gl, imageContext.blurProgram, imageContext.blurBuffer, temp.texture, [0, 1], radius, temp.width, temp.height);
};
