...
describe('drawGradientHandleLayer', () => {
  ...
  it('should draw nothing when no gradient editor is active', () => {
    // before
    drawGradientHandleLayer(CONTEXT, [rectangle()], null);
  ...
  it('should draw nothing when the active gradient editor targets a different node', () => {
    // before
    drawGradientHandleLayer(CONTEXT, [rectangle()], { nodeId: 'other', paintIndex: 0, selectedStopIndex: null });
  ...
  it('should draw nothing for a multi-node selection', () => {
    // before
    drawGradientHandleLayer(CONTEXT, [rectangle(), rectangle({ id: 'rect-2' })], {
      nodeId: 'rect-1',
      paintIndex: 0,
  ...
  it('should draw nothing when the targeted paint is not a linear gradient', () => {
    ...
    drawGradientHandleLayer(CONTEXT, [node], { nodeId: 'rect-1', paintIndex: 0, selectedStopIndex: null });
  ...
  it('should draw the line, endpoint handles, and stop handles for the targeted linear gradient fill', () => {
    // before
    drawGradientHandleLayer(CONTEXT, [rectangle()], { nodeId: 'rect-1', paintIndex: 0, selectedStopIndex: 1 });
