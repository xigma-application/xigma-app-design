...
export const drawScene = (
  ...
  const offsetVectorNode = offsetVector ? nodesById[offsetVector.nodeId] : undefined;
  const selectedNodes = offsetVectorNode ? [] : getVisibleSelectedNodes(allSelectedNodes, editingNodeId, refs, imageEditor);
  ...
  const hoveredNode = offsetVectorNode ? undefined : getVisibleHoveredNode(nodesById, hoveredNodeId, editingNodeId, refs);
  ...
  drawOffsetVectorPreview(ctx, offsetVector, nodesById);
  drawHoverOutline(ctx, offsetVectorNode, vectorEditingNodeIds, nodesById);
  drawOffsetVectorDistanceLabel(ctx, offsetVector, refs);
