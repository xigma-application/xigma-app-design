...
describe('getBoxStrokePolygons', () => {
  it('should return an outer and an inner polygon for the ring', () => {
    expect(getBoxStrokePolygons(rect(), 2, StrokeAlign.inside)).toHaveLength(2);
  });

  it('should put an inside stroke entirely within the node bounds', () => {
    const [outer, inner] = getBoxStrokePolygons(rect(), 2, StrokeAlign.inside);
  ...
  it('should put an outside stroke entirely outside the node bounds', () => {
    const [outer, inner] = getBoxStrokePolygons(rect(), 2, StrokeAlign.outside);
  ...
  it('should straddle the node bounds for a center stroke', () => {
    const [outer, inner] = getBoxStrokePolygons(rect(), 2, StrokeAlign.center);
  ...
  it('should default to an inside stroke when no alignment is set', () => {
    expect(getBoxStrokePolygons(rect(), 2, undefined)).toEqual(getBoxStrokePolygons(rect(), 2, StrokeAlign.inside));
  });

  it('should collapse the inner polygon instead of inverting it when the stroke is thicker than the node', () => {
    const [, inner] = getBoxStrokePolygons(rect({ height: 6, width: 6 }), 10, StrokeAlign.inside);
  ...
  it('should keep the outer polygon rounded further out than the inner one for a rounded node', () => {
    const [outer, inner] = getBoxStrokePolygons(rect({ cornerRadius: 10 }), 4, StrokeAlign.outside);
  ...
  it('should rotate both polygons around the node center', () => {
    const [outer] = getBoxStrokePolygons(rect({ rotation: 90 }), 2, StrokeAlign.inside);
