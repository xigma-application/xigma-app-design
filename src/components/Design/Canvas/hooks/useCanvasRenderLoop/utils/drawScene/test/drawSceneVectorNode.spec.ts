...
describe('drawSceneVectorNode', () => {
  ...
  it('should draw the node normally when there is no snapshot map at all', () => {
    ...
    const dragSnapshotFaceBufferCache = new WeakMap<TPoint[], WebGLBuffer>();
    const dragSnapshotStrokeBufferCache = new WeakMap<number[], WebGLBuffer>();
  ...
  it('should draw the frozen drag snapshot instead of the live node when one exists for this node’s id', () => {
    ...
    // result
    expect(drawVectorNodeDragSnapshotMock).toHaveBeenCalledWith(
      gl,
      dragSnapshotProgram,
      buffer,
      dragSnapshotFaceBufferCache,
