...
import { handleSelectMatchingLayers } from './utils/handleSelectMatchingLayers';
...
export const useKeyboardShortcuts = (refs: TCanvasRefs): void => {
  ...
  const keysMap: TKeysMap = useMemo(
    () => [
      ...
      { action: (): any => handleSelectMatchingLayers(dispatch), ...shortcuts.selectMatchingLayers },
      ...
      { action: (): any => handleZoomIn(dispatch, refs), ...shortcuts.zoomInNumpad },
      { action: (): any => handleZoomOut(dispatch, refs), ...shortcuts.zoomOut },
      { action: (): any => handleZoomOut(dispatch, refs), ...shortcuts.zoomOutNumpad },
