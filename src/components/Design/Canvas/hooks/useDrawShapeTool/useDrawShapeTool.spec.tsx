...
describe.each(CONFIGS)('useDrawShapeTool behaviors ($label)', ({ config }) => {
  it('should not react to pointer events when the tool is not active', () => {
    ...
    // before
    renderHook(() => useDrawShapeTool(createCanvasRefs({ canvasRef }), config), {
  ...
  it('should create the node immediately at pointer-down and resize it live while dragging', () => {
    // mock
    const store = createTestStore();
    ...
    // result — a real node already exists in the tree right after pointer-down
    ...
    // result — the very same node grows live as the pointer moves
  ...
  it('should select the newly created node immediately at pointer-down, replacing any existing selection', () => {
    // mock
    const store = createTestStore();
...
describe('useDrawShapeTool alignment snap', () => {
  ...
  it('should snap the in-progress node onto a nearby existing shape while dragging, populating the alignment guide', () => {
