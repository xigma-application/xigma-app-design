...
describe('handlePointerUp', () => {
  ...
  it('should commit a new page guide dropped outside the gutter', () => {
    // mock
    const canvas = createCanvas();
    const draggingGuideRef = { current: { axis: 'x', frameId: null, id: null, position: 50 } as TGuideDragState | null };
  ...
  it('should commit the new position of a moved guide', () => {
    ...
    const canvas = createCanvas();
    const draggingGuideRef = { current: { axis: 'x', frameId: null, id: guide.id, position: 250 } as TGuideDragState | null };
    const refs = createCanvasRefs({ guides: { draggingGuideRef } });
  ...
  it('should bracket the commit in a single undo step', () => {
    ...
    const guidesBefore = selectActivePage(store.getState()).guides;
    const canvas = createCanvas();
    const draggingGuideRef = { current: { axis: 'x', frameId: null, id: null, position: 70 } as TGuideDragState | null };
    const refs = createCanvasRefs({ guides: { draggingGuideRef } });
