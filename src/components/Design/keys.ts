// others
import { ALT, CONTROL, SHIFT } from 'constant/mainKeys';

// types
import { TKeyboardShortcuts } from './types';
import { ToolName } from 'types/design/enums';

export const KEYBOARD_SHORTCUTS: TKeyboardShortcuts = {
  addAutoLayout: [SHIFT, 'A'],
  alignBottom: [ALT, 'S'],
  alignHorizontalCenters: [ALT, 'H'],
  alignLeft: [ALT, 'A'],
  alignRight: [ALT, 'D'],
  alignTop: [ALT, 'W'],
  alignVerticalCenters: [ALT, 'V'],
  annotations: [SHIFT, 'Y'],
  [ToolName.arrow]: [SHIFT, 'L'],
  bold: [CONTROL, 'B'],
  booleanExclude: [ALT, SHIFT, 'E'],
  booleanIntersect: [ALT, SHIFT, 'I'],
  booleanSubtract: [ALT, SHIFT, 'S'],
  booleanUnion: [ALT, SHIFT, 'U'],
  bringForward: [CONTROL, ']'],
  bringToFront: [']'],
  bulletedList: [SHIFT, CONTROL, '8'],
  collapseLayers: [ALT, 'L'],
  [ToolName.comment]: ['C'],
  comments: [SHIFT, 'C'],
  convertToSlot: [SHIFT, CONTROL, 'S'],
  copy: [CONTROL, 'C'],
  copyAsPng: [SHIFT, CONTROL, 'C'],
  copyProperties: [ALT, CONTROL, 'C'],
  createComponent: [ALT, CONTROL, 'K'],
  createLink: [SHIFT, CONTROL, 'U'],
  [ToolName.cut]: ['X'],
  decreaseFontSize: [SHIFT, CONTROL, '<'],
  decreaseFontWeight: [ALT, CONTROL, '<'],
  decreaseIndentation: [CONTROL, '['],
  decreaseLetterSpacing: [ALT, '<'],
  decreaseLineHeight: [ALT, SHIFT, '<'],
  [ToolName.default]: ['V'],
  delete: ['⌫'],
  deleteAndHealSelection: [SHIFT, '⌫'],
  detachInstance: [ALT, CONTROL, 'B'],
  duplicate: [CONTROL, 'D'],
  [ToolName.ellipse]: ['O'],
  [ToolName.erase]: [SHIFT, 'E'],
  export: [SHIFT, CONTROL, 'E'],
  find: [CONTROL, 'F'],
  findNext: [SHIFT, CONTROL, 'F'],
  findNextFrame: ['End'],
  findPrevious: [SHIFT, CONTROL, 'D'],
  findPreviousFrame: ['Home'],
  flatten: [ALT, SHIFT, 'F'],
  flipHorizontal: [SHIFT, 'H'],
  flipVertical: [SHIFT, 'V'],
  [ToolName.frame]: ['F'],
  frameSelection: [ALT, CONTROL, 'G'],
  groupSelection: [CONTROL, 'G'],
  [ToolName.hand]: ['H'],
  hideShowLayer: [SHIFT, CONTROL, 'H'],
  increaseFontSize: [SHIFT, CONTROL, '>'],
  increaseFontWeight: [ALT, CONTROL, '>'],
  increaseIndentation: [CONTROL, ']'],
  increaseLetterSpacing: [ALT, '>'],
  increaseLineHeight: [ALT, SHIFT, '>'],
  italic: [CONTROL, 'I'],
  joinSelection: [CONTROL, 'J'],
  [ToolName.lasso]: ['Q'],
  layoutGuides: [SHIFT, 'G'],
  [ToolName.line]: ['L'],
  lockUnlockLayer: [SHIFT, CONTROL, 'L'],
  [ToolName.media]: [CONTROL, SHIFT, 'K'],
  multiplayerCursors: [ALT, CONTROL, '\\'],
  nextPage: ['↓'],
  numberedList: [SHIFT, CONTROL, '7'],
  openActions: [CONTROL, 'K'],
  openDesignPanel: [ALT, '8'],
  openLayersPanel: [ALT, '1'],
  openLibrariesPanel: [ALT, '2'],
  openPrototypePanel: [ALT, '9'],
  outlineStroke: [ALT, CONTROL, 'O'],
  [ToolName.paint]: [SHIFT, 'B'],
  pasteOverSelection: [SHIFT, CONTROL, 'V'],
  pasteProperties: [ALT, CONTROL, 'V'],
  pasteToReplace: [SHIFT, CONTROL, 'R'],
  [ToolName.pen]: ['P'],
  [ToolName.pencil]: [SHIFT, 'P'],
  pixelGrid: [SHIFT, "'"],
  pixelPreview: [SHIFT, CONTROL, 'P'],
  placeImage: [SHIFT, CONTROL, 'K'],
  [ToolName.polygon]: [],
  previousPage: ['↑'],
  [ToolName.rectangle]: ['R'],
  redo: [SHIFT, CONTROL, 'Z'],
  removeFill: [ALT, '/'],
  removeStroke: [SHIFT, '/'],
  renameLayer: [CONTROL, 'R'],
  resizeToFit: [ALT, SHIFT, CONTROL, 'R'],
  rulers: [SHIFT, 'R'],
  saveToVersionHistory: [ALT, CONTROL, 'S'],
  [ToolName.scale]: ['K'],
  [ToolName.section]: [SHIFT, 'S'],
  selectAll: [CONTROL, 'A'],
  selectInverse: [SHIFT, CONTROL, 'A'],
  selectMatchingLayers: [ALT, CONTROL, 'A'],
  selectNone: ['↺'],
  sendBackward: [CONTROL, '['],
  sendToBack: ['['],
  showHideUi: [CONTROL, '\\'],
  showOutlines: [SHIFT, CONTROL, 'O'],
  [ToolName.slice]: ['S'],
  smoothJoinSelection: [SHIFT, CONTROL, 'J'],
  [ToolName.star]: [],
  strikethrough: [SHIFT, CONTROL, 'X'],
  swapFillAndStroke: [SHIFT, 'X'],
  switchToDevMode: [SHIFT, 'D'],
  [ToolName.text]: ['T'],
  textAlignCenter: [ALT, CONTROL, 'T'],
  textAlignJustified: [ALT, CONTROL, 'J'],
  textAlignLeft: [ALT, CONTROL, 'L'],
  textAlignRight: [ALT, CONTROL, 'R'],
  [ToolName.textOnPath]: [],
  toggleUiMinimized: [SHIFT, CONTROL, '\\'],
  underline: [CONTROL, 'U'],
  undo: [CONTROL, 'Z'],
  ungroupSelection: [CONTROL, '⌫'],
  wrapInNewSection: [CONTROL, 'S'],
  zoomIn: [CONTROL, '+'],
  zoomOut: [CONTROL, '-'],
  zoomTo100: [CONTROL, '0'],
  zoomToFit: [SHIFT, '1'],
  zoomToNextFrame: ['N'],
  zoomToPreviousFrame: [SHIFT, 'N'],
  zoomToSelection: [SHIFT, '2'],
};
