...
const check = (node: TTextNode, ...ancestors: TSceneNode[]): boolean =>
  canExportTextAsSvgRealText(node, Object.fromEntries(ancestors.map((ancestor) => [ancestor.id, ancestor])));

describe('canExportTextAsSvgRealText', () => {
  it('should allow a plain unrotated text', () => {
    expect(check(text())).toBe(true);
  });

  it('should allow line breaks and any character (no embedded/subsetted font to gate against)', () => {
