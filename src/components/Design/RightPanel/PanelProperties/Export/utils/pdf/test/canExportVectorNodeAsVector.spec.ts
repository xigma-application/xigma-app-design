...
describe('canExportVectorNodeAsVector', () => {
  ...
  it('should allow a visible uniform stroke without a width profile', () => {
    expect(canExportVectorNodeAsVector(vectorNode({ strokeWidth: 2, strokes: [{ color: '#000000', opacity: 100, type: 'solid' }] }), {})).toBe(true);
  });

  it('should reject a visible stroke that has a variable width profile', () => {
    expect(canExportVectorNodeAsVector(vectorNode({ strokeWidth: 2, strokes: [{ color: '#000000', opacity: 100, type: 'solid' }], widthProfile: { points: {} } }), {})).toBe(
      false,
    );
  });
