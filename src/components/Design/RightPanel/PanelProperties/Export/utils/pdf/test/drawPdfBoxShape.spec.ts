...
describe('drawPdfBoxShape', () => {
  ...
  it('should draw the fill with the paint opacity as a fraction', () => {
    // action
    drawPdfBoxShape(page, rectangle({ fills: [{ color: '#00ff00', opacity: 40, type: 'solid' }] }), {}, bounds, states);
  ...
  it('should multiply in the inherited opacity of the ancestors', () => {
    ...
    // action
    drawPdfBoxShape(page, rectangle({ parentId: 'p' }), { p: parent }, bounds, states);
  ...
  it('should draw the stroke rings after the fill when the node has a stroke', () => {
    // action
    drawPdfBoxShape(page, rectangle({ strokeWidth: 2, strokes: [{ color: '#0000ff', opacity: 100, type: 'solid' }] }), {}, bounds, states);
