...
export const getPdfFunctionBasedAlphaShadingPattern = (
  context: PDFContext,
  ...
  const program = `{ ${getPdfGradientGeometryProgram(gradientType, geometry, radiusRatio)} ${getPdfGradientAlphaLookupProgram(stops)} }`;
  const functionRef = context.register(
    context.stream(program, { Domain: [0, bounds.width, 0, bounds.height], FunctionType: 4, Range: [0, 1] } as never),
  );
