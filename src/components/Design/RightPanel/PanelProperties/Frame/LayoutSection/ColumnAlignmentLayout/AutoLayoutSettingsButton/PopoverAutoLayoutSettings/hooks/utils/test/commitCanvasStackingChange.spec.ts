...
// utils
import { commitCanvasStackingChange } from '../commitCanvasStackingChange';
...
describe('commitCanvasStackingChange', () => {
  it('should dispatch an updateNode action with the frame id and the selected value', () => {
    // mock
    const dispatch = vi.fn();

    // action
    commitCanvasStackingChange(dispatch, frame(), CanvasStacking.firstOnTop);

    // result
    expect(updateNode).toHaveBeenCalledWith({ changes: { canvasStacking: CanvasStacking.firstOnTop }, id: 'frame-1' });
  ...
  it('should not dispatch when no frame node is selected', () => {
    ...
    // action
    commitCanvasStackingChange(dispatch, undefined, CanvasStacking.firstOnTop);
