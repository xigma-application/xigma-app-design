...
describe('useSyncPatternSourcePickTarget', () => {
  ...
  it('should set the pattern source pick target when the picker is open on a pattern paint with node ids', () => {
    // before
    renderHook(() => useSyncPatternSourcePickTarget(['node-1'], 2, 'fills', true, true), { wrapper });

    // result
    expect(selectPatternSourcePickTarget(store.getState())).toEqual({ nodeIds: ['node-1'], paintIndex: 2, property: 'fills' });
  });
  ...
  it('should clear the target once it was set and the picker then closes', () => {
    ...
    expect(selectPatternSourcePickTarget(store.getState())).toEqual({ nodeIds: ['node-1'], paintIndex: 0, property: 'fills' });
  ...
  it('should clear the target when there are no node ids', () => {
    // before
    const { rerender } = renderHook(({ nodeIds }) => useSyncPatternSourcePickTarget(nodeIds, 0, 'fills', true, true), {
      initialProps: { nodeIds: ['node-1'] as string[] },
      wrapper,
    });
  ...
  it("should not clear a sibling fill's pattern source pick target (regression: switching from a higher-index pattern fill back to a lower-index one wiped it back to null, because the higher-index row's own cleanup ran after the lower-index row's activation and unconditionally cleared whatever was there)", () => {
    ...
    expect(selectPatternSourcePickTarget(store.getState())).toEqual({ nodeIds: ['node-1'], paintIndex: 1, property: 'fills' });
  ...
  it('should tag the target with the strokes property and not clear a fills target of the same index', () => {
    // before
    store.dispatch(setPatternSourcePickTarget({ nodeIds: ['node-1'], paintIndex: 0, property: 'fills' }));
    const { unmount } = renderHook(() => useSyncPatternSourcePickTarget(['node-1'], 0, 'strokes', true, true), { wrapper });

    // result
    expect(selectPatternSourcePickTarget(store.getState())).toEqual({ nodeIds: ['node-1'], paintIndex: 0, property: 'strokes' });
