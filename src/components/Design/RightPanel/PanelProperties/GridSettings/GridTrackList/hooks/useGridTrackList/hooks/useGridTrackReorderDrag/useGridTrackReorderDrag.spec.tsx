...
describe('useGridTrackReorderDrag', () => {
  ...
  it('should begin a drag anchored on the first source index, unmoved, and block the default', () => {
    const { result } = renderHook(() => useGridTrackReorderDrag(3, () => true));
    ...
    expect(result.current.dragState).toEqual({ dropIndex: 2, grabbedIndex: 2, hasMoved: false, sourceIndices: [2] });
  });

  it('should track the drop index from the pointer position, flag the move, and commit on release', () => {
  ...
  it('should report no movement when released without a pointermove', () => {
    const onReorder = vi.fn(() => true);
    const { result } = renderHook(() => useGridTrackReorderDrag(3, onReorder));
    ...
    expect(onReorder).toHaveBeenCalledWith([0, 1], 0, 0, false);
  });
