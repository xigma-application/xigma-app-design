...
import { useZoomInClick } from '../useZoomInClick';
...
describe('useZoomInClick', () => {
  it('should increase the viewport zoom when called', () => {
    // mock
    store.dispatch(setViewport({ x: 0, y: 0, zoom: 1 }));

    // before
    const { result } = renderHook(() => useZoomInClick(), { wrapper });

    // action
    result.current();

    // result
    expect(selectViewport(store.getState()).zoom).toBeGreaterThan(1);
  });
});
