...
describe('getIsReorderNoOp', () => {
  ...
  it('should never be a no-op when the drop depth changed, even at the same slot', () => {
    // action & result
    expect(getIsReorderNoOp([2], 2, true)).toBe(false);
    expect(getIsReorderNoOp([2], 3, true)).toBe(false);
  });

  it('should default to treating an unchanged depth as a plain slot-based no-op check', () => {
    // action & result
    expect(getIsReorderNoOp([2], 2, false)).toBe(true);
  });
});
