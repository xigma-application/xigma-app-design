...
// utils
import { getTransformTargetNodes } from '../getTransformTargetNodes';
...
describe('getTransformTargetNodes', () => {
  ...
  it('should expand selected groups to leaves and dedupe overlaps', () => {
    expect(getTransformTargetNodes([outer, a], nodesById).map((node) => node.id)).toEqual(['a', 'b']);
  });
});
