...
describe('getGroupSubtreeNodes', () => {
  ...
  it('should return the group followed by every descendant group and leaf', () => {
    expect(getGroupSubtreeNodes(outer, nodesById).map((node) => node.id)).toEqual(['outer', 'inner', 'a', 'b']);
  });

  it('should return a plain node as a single-item list', () => {
    expect(getGroupSubtreeNodes(loose, nodesById)).toEqual([loose]);
  });

  it('should skip child ids that no longer resolve', () => {
    expect(getGroupSubtreeNodes(group('g', ['gone']), {}).map((node) => node.id)).toEqual(['g']);
  });
});
