...
- [x] **Pen / vector** — nie klasyczny `start → points → close`, tylko prawdziwy Vector Network
      (`NodeType.vector`: `vertices`/`segments` z kubicznymi tangentami _na segmencie_, nie na
...
- [x] **Shape Builder** (`ToolName.shapeBuilder`, skrót `M` w dropdownzie "More" `VectorEditToolbar`) —
      ...
      samo na jednym face'ie; **Alt** odejmuje zamiast łączyć — kasuje tylko _wyłączną_ granicę
