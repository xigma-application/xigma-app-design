...
| 263 | A painted square keeps a fill on the region unaffected by the drag after a single-vertex drag turns it self-intersecting ("bowtie") — `filledFaceKeys` gets remapped onto the new, still-valid face key at the end of the gesture instead of pointing at a now-stale key (single node, not multi-vector-edit; the sliver directly bounded by the dragged vertex not overlapping the shape's own original footprint is a known, geometrically-justified limitation, not asserted either way here — see `remapFilledFaceKeys.ts`) |  ✅  |    ✅ `vector-edit.spec.ts`    |
