...
## Section nesting — click-through like a frame, label selection, drop restrictions

A Section is a real container (`childIds`). A section with children is click-through, exactly like a
top-level Frame with children: a plain click or hover on its content reaches that content directly, a
plain click on its empty body selects nothing, and only its name label selects the section itself. An
empty section is selected from its body. A Frame nested directly inside a Section keeps its own
click-through status (its parent isn't a frame), so a plain click reaches content inside that frame
directly. A Section can only sit on the page or inside another section — never in a frame or group.
...
| 540 | Thousands of plain rectangles render with the right colors on both sides of a retained-chunk boundary, gaps stay empty                                                                                                                         |  ✅  |     ✅ `rect-batching.spec.ts`      |
| 541 | A plain rectangle inside a clipping frame is cut off at the frame edge                                                                                                                                                                         |  ✅  |     ✅ `rect-batching.spec.ts`      |
| 542 | Editing one rectangle of a batched run updates it without changing its neighbours                                                                                                                                                              |  ✅  |     ✅ `rect-batching.spec.ts`      |
| 543 | A batched rectangle (opaque, translucent fill, node opacity, rotated, rounded, several or hidden fills, inside / center / outside / translucent / multi-paint strokes) is pixel-identical to the same rectangle drawn through the regular path |  ✅  |     ✅ `rect-batching.spec.ts`      |
| 544 | Rectangles inside a mask and inside a clipping frame with opacity are pixel-identical to the regular path                                                                                                                                      |  ✅  |     ✅ `rect-batching.spec.ts`      |
| 545 | A drop shadow follows its node when the node moves and is gone from the old place                                                                                                                                                              |  ✅  |    ✅ `effect-textures.spec.ts`     |
| 546 | Editing a shadow's color redraws it instead of reusing the cached texture                                                                                                                                                                      |  ✅  |    ✅ `effect-textures.spec.ts`     |
| 547 | Two nodes with identical shadows both draw it and only the edited one changes                                                                                                                                                                  |  ✅  |    ✅ `effect-textures.spec.ts`     |
