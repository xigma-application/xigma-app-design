...
| 682 | Rotating a group (or several selected nodes) turns the image fill of every vector in it; the vectors keep the turn in their rotation instead of having it baked into their points |  ✅  |   ✅ `vector-snapshot-image-fill.spec.ts`    |
| 684 | A rotated vector with an image keeps its image turned in vector edit mode, also after the first click (the rotation is baked into its points on entry and kept for the fill) |  ✅  |   ✅ `vector-edit-rotated.spec.ts`    |
| 685 | The Lasso picks a point of a rotated vector where it is drawn |  ✅  |   ✅ `vector-edit-rotated.spec.ts`    |
| 686 | Cutting a rotated vector with an image gives both pieces the turned image covering all of them, and new pieces keep the whole style of the original |  ✅  |   ✅ `vector-edit-rotated.spec.ts`    |
