...
| 31  | Pressing Cmd/Ctrl+Z with focus still inside a track's value field or mode dropdown actually undoes (those fields opt out of the panel-wide bypass), and a track selection that no longer matches what an undo restored resets to the axis default instead of staying put |  ✅  | ✅ `grid.spec.ts` |
