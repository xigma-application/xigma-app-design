...
| 9   | Arrow-key reorder: pressing the cross-axis key (Down for Horizontal+Wrap, Right for Vertical+Wrap — opposite axis mapping) moves a selected child into the next line, verified by actual on-screen row/column position, not just `childIds` order                                  |  ✅  |  ✅ `flow.spec.ts` (arrow-cross-horizontal / arrow-cross-vertical tests)  |
| 10  | Arrow-key reorder: the primary-axis key is blocked at the edge of a child's own line instead of auto-crossing into the next one                                                                                                                                                    |  ✅  |              ✅ `flow.spec.ts` (arrow-primary-blocked test)               |
| 11  | Arrow-key reorder: a non-contiguous multi-selection (an unselected sibling sits between two selected ones) blocks the whole gesture, nothing moves                                                                                                                                 |  ✅  |           ✅ `flow.spec.ts` (arrow-non-contiguous-blocked test)           |
| 12  | Arrow-key reorder: pressing the cross-axis key backward (Up) moves a child into the previous line when that line already has room, no eviction needed                                                                                                                              |  ✅  |            ✅ `flow.spec.ts` (arrow-cross-up-with-slack test)             |
| 13  | Arrow-key reorder: pressing Up evicts the previous line's item at the SAME index the moved child occupies in its own line (not always the line's tail), pushing the evicted item forward — this is what makes Up work at all for a line packed edge-to-edge with equal-sized items |  ✅  | ✅ `flow.spec.ts` (arrow-cross-up-evicts / arrow-up-swaps-by-index tests) |
| 14  | Arrow-key reorder: pressing Up is blocked outright when the child itself is too wide to ever fit in the previous line, even after evicting everything from it                                                                                                                      |  ✅  |              ✅ `flow.spec.ts` (arrow-cross-up-blocked test)              |
...
| 46  | Clicking a track pill's chevron on the canvas opens an on-canvas dropdown with the same three sizing options (Fixed / Hug / Fill), same labels and icons, as the panel's own `GridTrackRow` mode dropdown — not the Grid settings panel, which a chevron click no longer force-opens                                                                                                                                                                                                                    |  ✅  | ✅ `grid.spec.ts` |
| 47  | Opening the chevron menu on a track that's part of a multi-selection leaves the whole selection intact (a plain click elsewhere on the pill would collapse it to just that track), and picking a mode from the menu applies it to every selected track, matching the panel dropdown's own multi-selection fan-out                                                                                                                                                                                       |  ✅  | ✅ `grid.spec.ts` |
| 48  | The Flow row's "Toggle automatic positioning" button only shows for Grid flow (not Free form / Horizontal / Vertical), and clicking it flips the frame's `gridAutoPlacement`, reflected in the button's pressed state                                                                                                                                                                                                                                                                                   |  ✅  | ✅ `grid.spec.ts` |
