...
| 37  | ~~Selecting two tracks, reordering them together, then undoing brings **both** original tracks back selected at their restored positions~~ — **known regression**, no longer holds: the revision-keyed selection-restore mechanism was removed when track selection split into independent canvas/panel fields (#42); undo restores the track structure but not the selection, so the test is `test.skip`ped until a replacement mechanism exists |  ✅  | ⚠️ skipped `grid.spec.ts` |
