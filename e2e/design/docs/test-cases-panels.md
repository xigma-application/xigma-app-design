...
| 693 | A Union (or group) holding a vector turns as one: a vector turned only as part of it bakes the turn into its points (keeping its fill turned) so the Union keeps its frame, a rotated vector counts with its drawn shape in group bounds, and an image, video or pattern fill of a rotated Union turns with it |  ✅  | ✅ `boolean-operations.spec.ts` |
