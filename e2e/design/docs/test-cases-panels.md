...
| 690 | In vector edit mode, with a handle selected and no point, X/Y show and move the end of the handle (mirroring its opposite handle like a drag), while Mirroring and Corner radius (one-corner icon) show and set those of the point the handle comes out of |  ✅  |   ✅ `vector-panel.spec.ts`    |
| 691 | Several vectors in vector edit mode show the Vector edit panel as "N selected": an untitled section (Alignment, Position, Mirroring, Corner radius working on the points and handles of every edited vector), Layout with Dimensions and Spacing, Fill, Stroke and Selection colors; the Vector path panel of several vectors also shows Spacing |  ✅  |   ✅ `vector-panel.spec.ts`    |
| 692 | A vector outside vector edit mode can be selected with other layers: the mixed panel shows the sections they share (a group with vectors inside also keeps its sections), W/H resize the vectors with the rest and the stroke weight reaches the vectors |  ✅  |   ✅ `vector-panel.spec.ts`    |
| 693 | A Union (or group) holding a vector turns as one: a vector turned only as part of it bakes the turn into its points (keeping its fill turned) so the Union keeps its frame and centre, a vector counts with its tight bounds in the turned group frame, a rotated vector counts with its drawn shape in group bounds, and an image, video or pattern fill of a rotated Union turns with it |  ✅  | ✅ `boolean-operations.spec.ts` |
| 694 | Smart Selection gap and swap drags move a selected Union or group together with everything inside it (was: only the Union node moved, its shapes stayed and the gap did nothing) |  ✅  | ✅ `smart-selection.spec.ts` |
| 695 | A Union or group holding a vector turned on its own is framed around the drawn (turned) vector, and alignment, spacing, export and selection bounds use the drawn shape of a turned vector (was: its unturned points) |  ✅  | ✅ `boolean-operations.spec.ts` |
| 696 | An ellipse, polygon or star image or video fill (and stroke) can be edited on the canvas like a rectangle's: Crop enters crop mode (move, resize, rotate the image) and Tile shows its scale handles; its crop follows the shape when it is moved, resized or rotated |  ✅  | ✅ `fill-section.spec.ts` |
